...
const ParticlesBackground: React.FC<ParticlesBackgroundProps> = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ dpr: window.devicePixelRatio || 1, alpha: true, depth: false });
    const gl = renderer.gl;
    container.appendChild(gl.canvas);

    // FIX PRINCIPAL: Forzar que el canvas ocupe todo el contenedor
    gl.canvas.style.position = 'absolute';
    gl.canvas.style.top = '0';
    gl.canvas.style.left = '0';
    gl.canvas.style.width = '100%';
    gl.canvas.style.height = '100%';
    gl.canvas.style.pointerEvents = 'none'; // Para que no bloquee interacciones
    ...
    // Generación de partículas (igual que antes)
  ...
  }, [particleCount, particleColors, speed, particleBaseSize, moveParticlesOnHover, alphaParticles]);
